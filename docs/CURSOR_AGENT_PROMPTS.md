...
- Clean working tree at hand-off: At the end of every task, run `git status -sb`. If there are uncommitted changes from your work:
  - Stage and commit the intended changes on your current feature/docs branch with a clear message.
  - Revert any incidental changes you did not intend to keep (for example, accidental deletions or mass reformatting).
  - The final state before you finish must be a clean working tree (no unstaged or untracked changes) so the next agent doesn’t start on a dirty tree.
