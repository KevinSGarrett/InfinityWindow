...
- Pipeline: Plans → CRM → TODO_CHECKLIST/PROGRESS → implementation/tests. If CRM/docs/tests ever disagree with the latest plans, treat it as an alignment issue and update CRM/docs/tests (and the plans, if they are wrong) rather than accepting drift.
...
| Docs alignment & canonical docs guardrails | `Project_Plan_003_UPDATED.txt` | Implemented | Canonical docs are listed in `CANONICAL_DOC_PATHS` (`backend/app/api/main.py`) and surfaced via `/debug/docs_status`; guarded by `qa/tests_docs/test_docs_existence.py` (non-empty files) and `qa/tests_api/test_docs_status.py` (endpoint + payload). |
