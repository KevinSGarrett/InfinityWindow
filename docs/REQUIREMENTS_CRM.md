...
Skim-first map of requirement clusters, their status, and where to look in docs/code/tests. For matrices/evidence, see `docs/ALIGNMENT_OVERVIEW.md` and `docs/alignment_002/`.
...
- Code: `backend/app/llm/openai_client.py`, `backend/app/api/main.py` (`/debug/telemetry`), `frontend/src/App.tsx`.
...
- Docs: `docs/TODO_CHECKLIST.md` (§2, §6), `docs/PROGRESS.md` (2025-12-14 audit trail), `docs/tasks/AUTOMATION.md`, `docs/USER_MANUAL.md` (§6.2–6.3).
- Code: `backend/app/api/main.py`, `backend/app/db/models.py` (`Task.auto_notes`), `frontend/src/App.tsx`.
...
## Usage & telemetry dashboard — Status: Partial (Phase 1/2 shipped; Phase 3 future)
- Scope: Cards + charts for action/model/confidence/auto-mode routes, shared filters/time window, filtered JSON/CSV exports, inline empty/error/export-fallback states. Long-window persistence/analytics (Phase 3) not started.
...
- Scope: Centralized retrieval profiles for messages/docs/memory/tasks with env-configurable `top_k`/thresholds (Phase 0) applied across chat/search (Phase 1). Future: per-surface strategy selection, context shaping, telemetry-driven tuning.
- Docs: `docs/CONFIG_ENV.md` (Retrieval profiles/Phase 0), `docs/TODO_CHECKLIST.md` (§2), `docs/PROGRESS.md` (2025-12-17 entry), `docs/SYSTEM_OVERVIEW.md`.
...
- Scope: `local_root_path` must exist/be a directory; Files tab shows inline error/reset-to-root guidance for missing/invalid paths; backend confinement via `_safe_join`/`_get_project_root`.
- Docs: `docs/TODO_CHECKLIST.md` (§3 FS UX line), `docs/PROGRESS.md` (2025-12-16/17), `docs/USER_MANUAL.md` (§8.1).
- Code: `backend/app/api/main.py` (`/projects/{id}/fs/list`, safety helpers), `frontend/src/App.tsx` (Files tab error states).
...
- Tests: `qa/ingestion_probe.py`, `qa/run_smoke.py`, `docs/TEST_PLAN.md` B-Docs.
