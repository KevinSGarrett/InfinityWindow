# InfinityWindow – Progress Log (Recovery Baseline)

Purpose: track shipped behavior in the recovered repo at `C:\InfinityWindow_Recovery` and call out gaps against the design plans (`Project_Plan_003`, `Updated_Project_Plans/*`).

## High-level status (2025-12-10)
- Backend (FastAPI 0.3.0): `/chat` with message/doc/memory retrieval and stubbed LLM; conversation auto-create; usage logging; task auto-maintainer and decision auto-capture.
- Projects: create/list/update with required `local_root_path`; instructions + pinned note; conversation folders; **no archive/delete**.
- Tasks: auto add/complete/dedupe after chat + manual trigger; suggestions approve/dismiss; telemetry counters + recent actions (`/debug/telemetry`); task audit fields (`auto_confidence`, `auto_last_action`, `auto_notes`).
- Docs & search: text ingest + doc CRUD/delete; repo ingestion jobs (create/poll/cancel); semantic search over messages/docs/memory.
- Memory & decisions: CRUD + search for memory; decision log CRUD; pinned memories injected into chat context.
- Files & terminal: guarded fs list/read/write; AI file edit endpoint (`instruction|instructions`, optional diff/apply); terminal run (global + scoped) with injection guard v1; history placeholder empty (persistence future).
- Usage: per-conversation usage records (`/conversations/{id}/usage`) plus telemetry counters; no dashboard persistence/exports.
- Frontend: React SPA covering the main tabs; Playwright specs in `frontend/tests` cover chat/tasks/docs/files/terminal/usage basics.
- Tests present: `qa/tests_api` (projects, chat routing, tasks automation/noisy/audit, usage telemetry, ingestion). Docs guard tests are **not present** in this baseline.

## 2025-12-10 – Repo recovery after multi-branch meltdown
- Recovered from `C:\InfinityWindow_Backup\019` into a clean baseline at `C:\InfinityWindow_Recovery`; reset SQLite + Chroma stores.
- Docs/CRM realigned to the recovered implementation; archived-project/export/import/Autopilot/Blueprint items are marked design-only.
- Known gaps: no project archive, no export/import bundle, no usage dashboard persistence, terminal history persistence still absent, no docs guard tests.
- Suggested validation commands for this baseline:
  - `python -m pytest qa/tests_api` (set `LLM_MODE=stub`, `VECTORSTORE_MODE=stub`).
  - `python -m pytest qa/tests_api/test_ingestion_e2e.py` for ingestion happy path.
  - `npm run build --prefix frontend` for the UI build.
  - Docs guard tests do not exist here (future guardrail to re-add).

## Backlog highlights
- Add project archive/restore and `include_archived` listing; export/import bundles.
- Build a usage dashboard UI with persisted telemetry/analytics/exports.
- Persist terminal history and extend terminal guard coverage.
- Add retrieval debug endpoint and tune retrieval quality.
- Data-driven tuning of task automation (confidence, dependency/ordering); dependency graphing.
- Autopilot/Blueprint phases (manager/worker agents, plan graph) — design-only today.
...
- Smoke/API suite re-run in stub mode; keep green before resyncing main.
...
## 2025‑12‑10 – Terminal injection guard v1
- Backend: added `ensure_safe_terminal_command` guard to `/terminal/run` (global + scoped) blocking `rm -rf /`, `rm -rf *`, `rm --no-preserve-root`, `del /s /q` on drive roots, `format <drive>`, and shutdown/poweroff commands. Guard returns HTTP 400 with `Command blocked by terminal guard: …` and skips execution.
- QA: Added `qa/tests_api/test_terminal_guard.py` covering allowed (`echo ok`) and blocked (`rm -rf /`) runs. Tests: `python -m pytest ..\qa\tests_api\test_terminal_guard.py -q --disable-warnings`; `python -m pytest ..\qa\tests_api -q --disable-warnings`; `LLM_MODE=stub VECTORSTORE_MODE=stub make ci` (backend API suite + frontend build).
- Gaps: terminal history persistence remains future; guard is pattern-based v1.
