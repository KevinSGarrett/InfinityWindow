...
## 2025‑12‑10 – Autonomous TODO v2: review queue heuristics & UI
- Backend: review queue thresholds split auto-apply (high confidence) from queued suggestions (low confidence, dependency-blocked, or duplicate); dependency hints now block unsafe auto-closes; telemetry records auto_applied/queued/dismissed suggestions alongside manual vs auto paths.
- Frontend: Tasks tab review queue shows confidence, priority chips, dependency badges, and reasons; approve/dismiss requires a reason; Playwright `tasks-suggestions` and `tasks-confidence` cover the v2 flow.
- Branches: `feature/agent-a-autotodo-v2`, `feature/agent-b-autotodo-v2`.
- Validation: `qa/tests_api/test_tasks_automation_audit.py`, `qa/tests_api/test_usage_telemetry_dashboard.py`, Playwright `frontend/tests/tasks-suggestions.spec.ts` + `frontend/tests/tasks-confidence.spec.ts`; `python -m pytest qa/tests_docs/test_docs_existence.py qa/tests_api/test_docs_status.py` (docs guard suite currently absent) plus `make ci` with `LLM_MODE=stub`/`VECTORSTORE_MODE=stub`.
