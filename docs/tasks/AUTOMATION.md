...
- Telemetry-driven tuning for auto-mode/task automation (confidence UX polish, learned heuristics).
- Deeper priority/grouping refinement and full dependency graphing (beyond current hints).
- Review/approve UX hardening for low-confidence additions/completions (suggested-change queue).
- Usage/telemetry dashboard Phase 3: long-window persistence/analytics once APIs exist.
- Retrieval/context shaping: per-surface tuning and strategy selection beyond Phase 0 profiles.
