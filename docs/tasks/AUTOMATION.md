...
## Review queue heuristics (v2)
- High-confidence additions/completions auto-apply; low-confidence, dependency-blocked, or duplicate suggestions enter the review queue with an explicit reason (confidence/dependency/duplicate/noise).
- Dependency hints in suggestions block auto-closes and show as badges/notes; queue items carry the hint so reviewers understand why automation paused.
- Approve/dismiss requires a reason; approvals apply the suggested change and clear stale entries; dismissals log `auto_dismissed` telemetry and remove the suggestion.
- Archived projects are read-only for automation; review queue actions are disabled and surface a “read-only/archived” message instead of applying changes.

## Telemetry & usage
- Task counters: auto-added / auto-completed / auto-deduped plus suggestion counters (auto_applied/queued/dismissed/ignored) with confidence stats/buckets; recent actions list includes status/priority/blocked/auto_notes/confidence/action/timestamp/matched_text/task_group and `dependency_hint` when present.
- Cost: pricing table includes `gpt-5-nano`, `gpt-5-pro`, `gpt-5.1-codex`; Usage tab now shows non-zero totals.
- Usage tab (2025-12-12): filters (action/group/model) + time filter (all/last5/last10), action/model counts, JSON/CSV export for filtered recent actions; routing reason for auto mode when available; suggestion counters surface in dashboard cards and exports.
- Dev/test seed helper: `/debug/seed_task_action` can create a task and record an automation action (including auto_suggested/auto_dismissed) with confidence/notes for UI/QA.
