...
- **Task automation**: auto-added, auto-completed, auto-deduped, auto-suggested counts (broken down into auto_applied / queued / dismissed / ignored); confidence stats/buckets; recent actions (status/priority/group/blocked/matched_text/dependency hints).
...
2) **Phase 2 (charts + filters)**:
   - Implemented:
     ...
     - Suggestion counters (auto_applied/queued/dismissed/ignored) appear in task counters, recent actions, and the filtered JSON/CSV exports alongside auto-add/complete/dedupe.
...
- Recent actions show action, confidence, status/priority/group/blocked, dependency hints, and matched_text when present; suggestion actions include auto_applied/queued/dismissed/ignored.
